...
static DXGI_FORMAT GetDXGIFormat(DDS_PIXELFORMAT ddpf)
{
	...
	else if((ddpf.flags & DDS_ALPHA) || (ddpf.flags & DDS_ALPHAPIXELS))
	{
		if(8 == ddpf.RGBBitCount)
		{
...
dds_u32 dds_supported_format(DXGI_FORMAT fmt)
{
	...
	case DXGI_FORMAT_A8_UNORM: return DDS_FMT_M8;
	case DXGI_FORMAT_R8_UINT: return DDS_FMT_M8;
	case DXGI_FORMAT_R8_UNORM: return DDS_FMT_M8;
...
void dds_getinfo(dds_info* info, dds_image_info* plane)
{
	...
	*plane = info->image;
	...
	else if(fmt == DDS_FMT_M8)
	{
		plane->pitch = plane->width;
