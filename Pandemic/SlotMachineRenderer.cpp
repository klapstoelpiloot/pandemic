...
#define HOLD_DISABLED_COLOR		Color(100, 80, 10)
#define HOLD_ENABLED_COLOR		Color(255, 220, 60)
#define SPIN_COLOR				Color(255, 220, 60)
...
#define SPINS_LEFT_POS			Point(117, 20)
...
SlotMachineRenderer::SlotMachineRenderer() :
	...
	spinslefttext(Main::GetResources().GetFont("pixel7.fnt"), HorizontalAlign::Center, VerticalAlign::Bottom),
	spinslefttexture(Main::GetResources().GetImage("yellow12.dds"))
{
...
void SlotMachineRenderer::Render(Canvas& canvas)
{
	...
	for(int i = 0; i < NUM_WHEELS; i++)
	{
		...
		float wheelblur = std::clamp(static_cast<float>((wheelspeed[i] - BLUR_START_SPEED) / (BLUR_FULL_SPEED - BLUR_START_SPEED)), 0.0f, 1.0f);
		...
		int topicon = Modulo(roundedpos + 1, NUM_ICONS);
		int middleicon = Modulo(roundedpos, NUM_ICONS);
		int bottomicon = Modulo(roundedpos - 1, NUM_ICONS);
		...
		if(wheelblur < 1.0f)
		{
			// Draw normal wheel images
			canvas.DrawColorImageBlend(Point(xpos, ypos - ICON_Y_DISTANCE), *icons[topicon]);
			canvas.DrawColorImageBlend(Point(xpos, ypos), *icons[middleicon]);
			canvas.DrawColorImageBlend(Point(xpos, ypos + ICON_Y_DISTANCE), *icons[bottomicon]);
		}

		if(wheelblur > 0.0f)
		{
			// Draw blurred images
			blurscanvas.DrawColorImageBlend(Point(xpos, ypos - ICON_Y_DISTANCE), *bluricons[topicon]);
			blurscanvas.DrawColorImageBlend(Point(xpos, ypos), *bluricons[middleicon]);
			blurscanvas.DrawColorImageBlend(Point(xpos, ypos + ICON_Y_DISTANCE), *bluricons[bottomicon]);

			// Set the alpha values on the pixels for this blur
			byte blurbyte = static_cast<byte>(std::clamp(static_cast<int>(wheelblur * 255.0f), 0, 255));
	...
	// Spins remaining number
	spinslefttext.DrawOutlineBlend(canvas, SPINS_LEFT_POS, 1, BLACK);
	spinslefttext.DrawTexturedModBlend(canvas, SPINS_LEFT_POS, spinslefttexture, WHITE);
}
...
void SlotMachineRenderer::SetSpins(int sp)
{
	spinslefttext.SetText(String::From(sp));
}
